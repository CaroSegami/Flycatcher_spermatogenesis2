...
Load seurat objects (already subsetted)
...
fl.combined<-readRDS("fl.raw.combined_2022.rds")
...
#They all work fine but subset 3 has more cells, let's try with that one.
...
#save seurat obj
saveRDS(pure.combined.sct, file = "pure_clusteredsct.rds")
```
...
#save seurat obj
saveRDS(collareds.combined.sct, file = "collareds.combined.sct.rds")
...
p6
#save seurat obj
saveRDS(pieds.combined.sct, file = "pieds.combined.sct.rds")
...
```{r find}
pure.combined.sct<-readRDS("pure_clusteredsct.rds")
...
#Load new annotation
NewAnn<-read_delim("FinalNewAnnotation.tsv", delim=" ", col_names=T) 
NewAnn1<-NewAnn%>%
  select(X2,gene_name)
##Genes we are using in sc analysis
genes <- read.table("features.tsv.gz", stringsAsFactors = FALSE)
genes<-genes%>%dplyr::select(V1,V2)%>%distinct(V2, .keep_all = T)
NewAnn2<-merge(NewAnn1,genes, by.x="X2", by.y="V1")
NewAnn2$gene_name<-toupper(NewAnn2$gene_name)
write.csv(NewAnn2, file = "NewAnn2.csv")

#I think it's too complicated at this stage... will do later
...
#marker plot
plot_9<-DotPlot(pure.combined.sct, features = c("HSP90AA1", "HMGB1", "H2AFZ", "RPS24", "ENSFALG00000008965", "ENSFALG00000000048", "RPS11", "ENSFALG00000010973", "ENSFALG00000013662", "RPL23A"), cols = c("blue","red"), dot.scale = 8) + RotatedAxis()+NoLegend() +
   ylab("Cluster")+
  scale_x_discrete(labels = function(x) recode(
    x,
    "ENSFALG00000008965"= "RPL17",
    "ENSFALG00000000048"= "PPIA",
    "ENSFALG00000010973"="RPS11",
    "ENSFALG00000013662"="RPL32"))+
  theme(
    axis.text.x = element_text(face = "italic")
  )

plot_0<-DotPlot(pure.combined.sct, features = c("CDKN1B", "VIM", "COX1", "ENSFALG00000016233", "ENSFALG00000012594", "ENSFALG00000000295", "COX2", "WFS1", "ENSFALG00000014556", "ENSFALG00000000659"), cols = c("blue","red"), dot.scale = 8) + RotatedAxis()+NoLegend()+
   ylab("Cluster")+
  scale_x_discrete(labels = function(x) recode(
    x,
    "ENSFALG00000012594"= "CSNK1E",
    "ENSFALG00000000295"= "MED17",
    "ENSFALG00000014556"="GNG11",
    "ENSFALG00000000659"="RAP1A"))+
  theme(
    axis.text.x = element_text(face = "italic")
  )
...
plot_10<-DotPlot(pure.combined.sct, features = c("CALM2", "HSP90AA1", "UCHL1", "STMN1", "ENSFALG00000009451", "ASZ1", "DUT", "SMC2", "HMGB2", "ENSFALG00000009073"), cols = c("blue","red"), dot.scale = 8) + RotatedAxis()+
  ...
  scale_x_discrete(labels = function(x) recode(
    x,
    "ENSFALG00000009451"= "UQCRH",
    "ENSFALG00000009073"= "SYCP3"))+
  theme(
    axis.text.x = element_text(face = "italic")
  )

(plot_0|plot_9|plot_10) + plot_annotation(tag_levels = "A")
...
#saveRDS
saveRDS(pure.minus0.rec,"DS1DS2pureminus0.sct.rds")

#flminus0
pure.minus0.rec<-fl.minus0

pure.markers <- FindAllMarkers(pure.minus0.rec, only.pos = TRUE, min.pct = 0.25, logfc.threshold = 0.25)
write.csv(pure.markers, file = "pure.markers.csv")
...
```{r find2}
#collareds.combined.sct<-readRDS("collareds.combined.sct.rds")
...
#write.csv(collareds.markers,file = "CF_markers.csv")
...
```{r find2}
#pieds.combined.sct<-readRDS("pieds.combined.sct.rds")
...
DotPlot(pure.minus0.rec, features = c("CLU", "TAGLN","ACTA2", "CD34", "TM4SF1","APOE","CD74",
                                      ...
                                      "SYCP1","PIWIL1","SYCP2","TANK","AURKA","ENSFALG00000010544","ACRV1","SPACA1",
                                      "SPATA3","NRBP1","PRM1","GABBR2"), 
        cols = c("blue","red"), dot.scale = 8) + RotatedAxis()
#ENSFALG00000010544=LRRIQ1
#####More markers
spermatogonia.markers<-c("ID4","L1TD1","ZBTB43","NR6A1","DMRT1","DAZL","SYCP3","BEND4","MORC1","KIT","ZBTB16","ITGA6","LIN28",
                         "DPPA4","FGFR3","SOHLH1","SOHLH2","REC8","TEX101","LY6K","TSPAN33","HELLS","NANOS3","NANOS2","NEUROG3",
                         "LIN28A","UTF1","SCML2","RAD51","RHOX13","ETV5","RHOXF1","MAGEA4","BCL6B","GFRA1","PIWIL4","STRA8",
                         "ENSFALG00000008893","ENSFALG00000009073","ENSFALG00000006160")

#DAZL=ENSFALG00000008893
#SYCP3=ENSFALG00000009073
#NANOS3=ENSFALG00000006160
spermatocyte.markers<-c("SYCP1","SYCP2","CENPU","MEIOB","SPATA22","NOL8","ACYP1","HORMAD1","MYBL1","DMC1","FBXO47","SOX30", 
                        "TOP2B", "SYCE1","PIWIL1","TANK","AURKA","LDHA")

spermatid.markers<-c("UNC80","ACRV1","H1FNT","SPACA3","PRM1","PRM2","TNP1","SPATA3","TNP2","TPPP2","CBY3","SPACA4","CHD5","SPATA19",
                     "IQCF6","PEX5L","ABHD5","REEP6","TCT24","LRRIQ1","SPACA1","CCNB1","TSSK6","CETN4","TEKT1","ACR","SSXB1","SSXB2",
                     "CATSPER3","CATSPER1","GABBR2","ENSFALG00000010544")
#ENSFALG00000010544=LRRIQ1

macrophage.markers<-c("CD74","CSF1R","PECAM1","LYZ","TYROBP","CD83","PFN1","C1QC","CTSS","PF4","FCER1G","CBR2",
                      "CTSC","WFDC17","LAPTM5","CTSB","ITM2B","ENSFALG00000002762","ENSFALG00000004560")
#LYZ=ENSFALG00000002762
#CTSS=ENSFALG00000004560
...
sommatic.markers<-c("IGFBP5","IGFBP7","COL4A1","NR2F2","CXCL12","ACTA2","MYH11","MYL6","CD52","CD69","CD3D","TRAC",
                    ...
                    "CFD","DCN","DLK1","IGF1","IGF2","SFRP1","RPL10","ENSFALG00000014328","ENSFALG00000007904","ENSFALG00000003566",
                    "ENSFALG00000004561","ENSFALG00000010740","ENSFALG00000014458")
#CD69=ENSFALG00000014328
#SELE=ENSFALG00000007904
#CRIP1=ENSFALG00000003566
#ITGA1=ENSFALG00000004561,ENSFALG00000010740
#CHCHHD10=ENSFALG00000014458

DotPlot(pure.minus0.rec, features = (spermatogonia.markers), cols = c("blue","red"), dot.scale = 8) + RotatedAxis()+
  ylab("Cluster")+
  scale_x_discrete(labels = function(x) recode(
    x,
    "ENSFALG00000008893"= "DAZL",
    "ENSFALG00000009073"= "SYCP3"))+
  theme(
    axis.text.x = element_text(face = "italic")
  )
...
DotPlot(pure.minus0.rec, features = c("DDX4"), cols = c("blue","red"), dot.scale = 8) + RotatedAxis()  #Lau DDX4 germ and VIM somatic (in mammals)
...
Dimminus0
cf.combined.sct.minusSOM<-subset(cf.minus0.rec, idents = c("18","2","9","19"), invert=T)
...
#save seurat obj
saveRDS(cf.minusSOM.rec, file = "cf.minusSOM.rec.rds")
...
#save seurat obj
saveRDS(pf.minusSOM.rec, file = "pf.minusSOM.rec.rds")
...
#save seurat obj
saveRDS(fl.minusSOM.rec, file = "fl.minusSOM.rec.rds")

minusSOM.markers <- FindAllMarkers(fl.minusSOM.rec, only.pos = TRUE, min.pct = 0.25, logfc.threshold = 0.25)
write.csv(minusSOM.markers = "minusSOM.markers.csv")

minusSOM.top100 <- minusSOM.markers %>% group_by(cluster) %>% top_n(n = 100, wt = avg_log2FC)

#This needs to be done in the cluster
#DefaultAssay(pf.minus0.rec) <- "RNA"
#SaveH5Seurat(pf.minus0.rec, filename = "fl2comb2019.h5Seurat")
