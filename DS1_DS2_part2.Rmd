...
fl.minus0<-readRDS("DS1DS2pureminus0.sct.rds")
fl.combined.markers<-read.csv("pure.markers.csv")
fl.top100 <- fl.combined.markers %>% group_by(cluster) %>% top_n(n = 100, wt = avg_log2FC)
fl.top100$cluster<-as.factor(fl.top100$cluster)
...
cf.minus0.rec<-readRDS("cf.minus0.sct.rds")
cf.markers<-read.csv("cfminus0.markers.csv") #load new version
cf.top100 <- cf.markers %>% group_by(cluster) %>% top_n(n = 100, wt = avg_log2FC)
cf.top100$cluster<-as.factor(cf.top100$cluster)
cf.top10 <- cf.markers %>% group_by(cluster) %>% top_n(n = 10, wt = avg_log2FC)
cf.markers_signif<-cf.markers%>%filter(p_val_adj<0.05)

Dimminus02<-DimPlot(cf.minus0.rec, reduction = "umap", label = TRUE)+NoLegend()
Dimminus02


pf.minus0.rec<-readRDS("pf.minus0.sct.rds")
pf.markers<-read.csv("pf.markers.csv")
pf.top100 <- pf.markers %>% group_by(cluster) %>% top_n(n = 100, wt = avg_log2FC)
pf.top100$cluster<-as.factor(pf.top100$cluster)
pf.top10 <- pf.markers %>% group_by(cluster) %>% top_n(n = 10, wt = avg_log2FC)
pf.markers_signif<-pf.markers%>%filter(p_val_adj<0.05)

Dimminus03<-DimPlot(pf.minus0.rec, reduction = "umap", label = TRUE)+NoLegend()
Dimminus03

NewAnn2<-read.csv("NewAnn2.csv")
NewAnn2$gene_name<-toupper(NewAnn2$gene_name)
NewAnn2<-select(NewAnn2, X2,gene_name,V2)
fl.combined.markers<-select(fl.combined.markers, gene, p_val, avg_log2FC, p_val_adj,cluster)
Markers_correspondance<-merge(fl.combined.markers, NewAnn2, by.x="gene", by.y="V2", all.x=T, all.y=F)
Mfl.markers.signif<-filter(Markers_correspondance,p_val_adj<0.05)
#write.csv(Mfl.markers.signif, file = "pure.signif.markers.csv")

level_order_cf<-c("18","2","9","19","16","4","8","10","14","15","6","3","12","0","17","7","11","13","1","5")
...
levels(fl.top100)<-level_order_pure

library(reshape2)
library(corrplot)

umaps<-plot_grid(Dimminus01, nrow = 2, ncol = 2)
umaps
...
level_order_cf<-c("18","2","9","19","8","10","14","15","6","16","4","3","12","0","17","7","11","13","1","5")
levels(cf.minus0.rec)<-level_order_cf
levels(cf.top100)<-level_order_cf

level_order_pf<-c("19","17","4","14","16","18","11", "10", "15", "9","12","2","3","13","1","5","6","7","0","8") #good order good in  velo
levels(pf.minus0.rec)<-level_order_pf
levels(pf.top100)<-level_order_pf
...
flmarkers.cfpure$cluster.x <- factor(as.character(flmarkers.cfpure$cluster.x), levels = level_order_cf)
flmarkers.cfpure$cluster.y <- factor(as.character(flmarkers.cfpure$cluster.y), levels = level_order_pure)
mat <- table(DS1 = flmarkers.cfpure$cluster.x, DS2 = flmarkers.cfpure$cluster.y)
mat <- mat[level_order_cf, level_order_pure, drop = FALSE]

corr_plot2<-corrplot(mat,is.corr=F,
                     method = 'color',
                     number.digits = 0, addCoef.col  = "black")

flmarkers.pfpure<-merge(pf.top100,fl.top100, by.x="gene", by.y="gene")
flmarkers.pfpure$cluster.x <- factor(as.character(flmarkers.pfpure$cluster.x), levels = level_order_pf)
flmarkers.pfpure$cluster.y <- factor(as.character(flmarkers.pfpure$cluster.y), levels = level_order_pure)
mat2 <- table(DS1 = flmarkers.pfpure$cluster.x, DS2 = flmarkers.pfpure$cluster.y)
mat2 <- mat2[level_order_pf, level_order_pure, drop = FALSE]


corr_plot3<-corrplot(mat2,is.corr=F,
                     method = 'color',
                     number.digits = 0,
                     addCoef.col  = "black")


flmarkers.cfpf<-merge(cf.markers_signif,pf.markers_signif, by.x="gene", by.y="gene")
flmarkers.cfpf$cluster.x <- factor(as.character(flmarkers.cfpf$cluster.x), levels = level_order_cf)
flmarkers.cfpf$cluster.y <- factor(as.character(flmarkers.cfpf$cluster.y), levels = level_order_pf)
mat3 <- table(DS1 = flmarkers.cfpf$cluster.x, DS2 = flmarkers.cfpf$cluster.y)
mat3 <- mat3[level_order_cf, level_order_pf, drop = FALSE]

corr_plot4<-corrplot(mat3,is.corr=F,
                     method = 'color',
                     number.digits = 0)
...
corrplot2<-ggplotify::as.ggplot(~corrplot(mat,is.corr=F,
                     method = 'color',
                     number.digits = 0, addCoef.col  = "black"))

corrplot3<-ggplotify::as.ggplot(~corrplot(mat2,is.corr=F,
                     method = 'color',
                     number.digits = 0,
                     addCoef.col  = "black"))

corrplot4<-ggplotify::as.ggplot(~corrplot(mat3,is.corr=F,
                     method = 'color',
                     number.digits = 0))
#plots
Dimminus02|corrplot2
Dimminus03|corrplot3

(Dimminus02|corrplot2)/(Dimminus03|corrplot3)
(Dimminus02/Dimminus03)|corrplot4
...
Heatmap<-DoHeatmap(heatmap, features = fl.top10$gene) + theme(axis.text.y = element_text(size =2), axis.text.x = element_text(size=6))
all_heat<-Heatmap + scale_fill_gradient2(low = "navy", mid = "white",high = "red") +NoLegend()
...
per <- df  %>%  group_by(cluster) %>% 
  dplyr::count(dataset) %>% 
  ungroup()
#write.csv(per, file = "number_cells_perclus_ind.csv")
p<-ggplot(data=per, aes(x=cluster, y=n, fill=dataset)) +
  geom_bar(position="fill", stat="identity")+ labs(y = "Percent") +  scale_y_continuous(labels = scales::percent) + scale_fill_viridis(discrete = TRUE, option = "H") + theme_classic()
p

pp<-ggplot(data=per, aes(x=cluster, y=n, fill=dataset)) +
  geom_bar( stat="identity")+ labs(y = "Count") + scale_fill_viridis(discrete = TRUE, option = "H") + theme_classic()
...
#write.csv(top10_markers_roc,file = "top10markers_roc_pure.csv")
...
#write.csv(All_rocmarkers,file = "All_rocmarkers_pure.csv")
All_rocmarkers<-read.csv(file="All_rocmarkers_pure.csv",sep=",")

All_rocmarkers<-merge(All_rocmarkers, NewAnn2, by.x="Gene_name", by.y="V2", all.x=T, all.y=F)
...
best_noL<-bestmarkers + NoLegend() +xlab("Gene names")+ylab("Cluster") + theme(axis.text.x = element_text(size=7), axis.text.y = element_text(size=7))
...
genes <- read.table("filtered_feature_bc_matrix_CF1/features.tsv.gz", stringsAsFactors = FALSE)
...
#write.csv(fl.markers.signif,file = "fl.markers-pure.signif.csv")
```
...
#clusdiffresponse_wilcox_adj<-read.csv(file = "DEwilcox_cfpf_significant.csv")

Diff_genes_GO<-distinct(clusdiffresponse_wilcox_adj, genes)
Diff_genes_GO<-merge(Diff_genes_GO, NewAnn2, by.x="genes", by.y="V2", all.x=T, all.y=F)
write.csv(Diff_genes_GO, file="list_allDEids.csv")
Diff_genes_GO_perCluster<-select(clusdiffresponse_wilcox_adj, genes,cluster)
Diff_genes_GO_perCluster<-merge(Diff_genes_GO_perCluster,NewAnn2, by.x="genes", by.y="V2", all.x=T, all.y=F)
#write.csv(Diff_genes_GO_perCluster,file = "Diff_genes_GO_perClus.csv")

#Diff_genes_GO<-read.csv("list_allDEids.csv")
...
totalgenes<-c("3688","3688","4941","4941","6899","6899","6829","6829","8225","8225",
              "6346","6346","6585","6585","6908","6908",
              "6830","6830","6448","6448","7398","7398","5712","5712","7352","7352","5912","5912","6424","6424",
              "7119","7119","5945","5945","5665","5665","5860","5860","5503","5503", "5606","5606")
...
cluster<-c("20","20","19","19","12","12","5","5","17","17","9","9","8","8",
           "14","14","15","15","7","7","11","11","2","2","18","18","4","4","16","16","10","10",
...
nDE<-c("0","0","1","0","4","0","36","1","3","0","4","0","33","4","1","0","2","0","46","3",
       "82","2","66","4","87","6","77","7","2","2","345","19","273","19","21","2","194","14",
       "369","26","146","10")
...
DEtotal<-c("0","0","1","1","4","4","37","37","3","3","4","4","37","37","1","1","2","2",
           "49","49","84","84","70","70","93","93","84","84","4","4","364","364","292","292",
...
avgexp_newann<-merge(avg.cells.all2, NewAnn2, by.x="gene", by.y="V2", all.x=T, all.y=F)
avgexp_newann<-na.omit(avgexp_newann)
expmore0<-avgexp_newann%>%
  filter(all>0.01)
write.csv(expmore0, file = "exp_genes.csv")
#write.csv(avg.cells.all2,file = "avg_exppure.csv")
...
#write.csv(avg.cells.all2,file = "avg_top500pure.csv")
...
###Here we do bootstrap resampling for the average expression graphs.
library(data.table)
library(reshape2)
...
#write.csv(avg.clus.cells2,file = "avgexp_all_a.csv")
...
#write.csv(longdfexp_top500,file = "Top500_expressed.csv")
...
p2<-c(0.7661754,0.4192847,0.5093431,0.687296,0.4192847,0.06618954,0.6002477,0.3344532,0.005821254,0.0275343,0.6002477,0.3344532,0.09773867,0.3344532,0.193094,0.5093431,0.1396615,0.4192847,0.7661754,0.06618954,0.4192847)

#Trying different methods
p.adjust(p2, method = "bonferroni", 21)
p.adjust(p2, method = "holm", 21) 
...
dnds<-read.table("/Users/carosegami/Downloads/dNdS_Fice_3spalignment_frm_unfiltered.txt", 
                 sep = "\t", header = T)
...
Human <- read.table("THE_FINAL_CLUSTERS/Fig_1B_Human_unselected_Hermann2018.csv", sep = ",", header = T)

Mouse$cluster<-as.factor(Mouse$cluster)
Human<-Human%>%
...
fl.markers<-merge(Mfl.markers.signif,stages, by.x = "cluster", by.y = "cluster")
fl.markers$stage<-as.factor(fl.markers$stage)
##Human
...
spermatogoniaV <- list(
  fly=fly_spermatogonia$gene_name,
...
# Clean first: unique + drop NAs to match venn tools' counting
sets_spe <- lapply(spermatogoniaV, \(x) unique(na.omit(x)))
fly1 <- sets_spe$fly 
hum1 <- sets_spe$hum 
mou1 <- sets_spe$mou

# “z only” = fly only (in fly, not in mou or hum)
fly1_only <- setdiff(fly1, union(mou1, hum1))
writeLines(fly1_only, "spermatogonia_fly_only.txt")
shared_all <- Reduce(intersect, list(fly1, hum1, mou1))
writeLines(shared_all, "spermatogonia_shared_all.txt")
shared_hummou <- setdiff(intersect(mou1,hum1), fly1)
writeLines(shared_hummou, "spermatogonia_shared_hummou.txt")
humplusmouminusfly <- setdiff(union(mou1,hum1), fly1)
writeLines(humplusmouminusfly, "hummouminusfly.txt")
...
write.csv(fly_meiosis,"fly_meio_signif_markers.csv")

meio <- list(
  fly=fly_meiosis$gene_name,
...
# Clean first: unique + drop NAs to match venn tools' counting
sets_meio <- lapply(meio, \(x) unique(na.omit(x)))
fly2 <- sets_meio$fly 
hum2 <- sets_meio$hum 
mou2 <- sets_meio$mou

# “z only” = fly only (in fly, not in mou or hum)
fly2_only <- setdiff(fly2, union(mou2, hum2))
writeLines(fly2_only, "meio_fly_only.txt")
shared_all2 <- Reduce(intersect, list(fly2, hum2, mou2))
writeLines(shared_all2, "meio_shared_all.txt")
shared_hummou2 <- setdiff(intersect(mou2,hum2), fly2)
writeLines(shared_hummou2, "meio_shared_hummou.txt")
humplusmouminusfly2 <- setdiff(union(mou2,hum2), fly2)
writeLines(humplusmouminusfly2, "meio_hummouminusfly.txt")
...
# Clean first: unique + drop NAs to match venn tools' counting
sets_elo <- lapply(elo, \(x) unique(na.omit(x)))
fly3 <- sets_elo$fly 
hum3 <- sets_elo$hum 
mou3 <- sets_elo$mou

# “z only” = fly only (in fly, not in mou or hum)
fly3_only <- setdiff(fly3, union(mou3, hum3))
writeLines(fly3_only, "elo_fly_only.txt")
shared_all3 <- Reduce(intersect, list(fly3, hum3, mou3))
writeLines(shared_all3, "elo_shared_all.txt")
shared_hummou3 <- setdiff(intersect(mou3,hum3), fly3)
writeLines(shared_hummou3, "elo_shared_hummou.txt")
humplusmouminusfly3 <- setdiff(union(mou3,hum3), fly3)
writeLines(humplusmouminusfly3, "meio_hummouminusfly.txt")
...
biovenn_spermatogonia <- draw.venn(fly_spermatogonia$gene_name, hum_spermatogonia$gene, 
                                   mou_spermatogonia$gene, subtitle="Spermatogonia", nrtype="abs")

biovenn_meiosis <- draw.venn(fly_meiosis$gene_name, hum_meiosis$gene, 
                                   mou_meiosis$gene, subtitle="Spermatocytes", nrtype="abs")

biovenn_elongating <- draw.venn(fly_elongating$gene_name, hum_elongating$gene, 
...
x<-as.table(rbind(c(1387, 849), c(1745,1918), c(568,767)))
...
# New Venn Diagram between CF and PF

```{r}

level_order_cf<-c("18","2","9","8","11","14","16","6","15","4","3","12","0","17","7","10","13","1","5") #good order translate to velo
level_order_pf<-c("19","17","16","18","14","4","11", "10", "15", "9","12","2","3","13","1","5","6","7","0","8") #good order good in  velo

cf.markers_signif<-cf.markers %>%
  filter(p_val_adj<0.05)
cf.markers_signif<-merge(cf.markers_signif,NewAnn2, by.x="gene",by.y="V2", all.x=T, all.y=F)
cf.markers_signif<-na.omit(cf.markers_signif)

pf.markers_signif<-pf.markers %>%
  filter(p_val_adj<0.05)
pf.markers_signif<-merge(pf.markers_signif,NewAnn2, by.x="gene",by.y="V2", all.x=T, all.y=F)
pf.markers_signif<-na.omit(pf.markers_signif)

cluster_cf<-c("18","2","9","8","11","14","16","6","15","4","3","12","0","17","7","10","13","1","5") 
stage_cf<-c("somatic","somatic","somatic","spermatogonia","spermatogonia",
            "meiosis","meiosis", "meiosis","meiosis","meiosis", "meiosis","meiosis",
            "elongating", "elongating", "elongating","elongating","elongating","elongating","elongating")

stages_cf<-cbind(cluster_cf,stage_cf)
stages_cf<-as.data.frame(stages_cf)

CF_forvenn<-merge(cf.markers_signif,stages_cf, by.x = "cluster", by.y = "cluster_cf", all = T)
CF_forvenn$stage_cf<-as.factor(CF_forvenn$stage_cf)
#PF

cluster_pf<-c("19","17","16","18","14","4","11", "10", "15", "9","12","2","3","13","1","5","6","7","0","8")
stage_pf<-c("somatic","somatic","somatic","somatic","somatic","somatic",
            "spermatogonia","spermatogonia",
            "meiosis","meiosis","meiosis","meiosis","meiosis","meiosis",
            "elongating","elongating","elongating","elongating","elongating","elongating")

stages_pf<-cbind(cluster_pf,stage_pf)
stages_pf<-as.data.frame(stages_pf)

PF_forvenn<-merge(pf.markers_signif,stages_pf, by.x = "cluster", by.y = "cluster_pf", all = T)
PF_forvenn$stage_pf<-as.factor(PF_forvenn$stage_pf)

CF_forvenn<- CF_forvenn %>% group_by(stage_cf) %>% arrange(avg_log2FC)%>%distinct(gene, .keep_all = T)
PF_forvenn<- PF_forvenn %>% group_by(stage_pf) %>% arrange(avg_log2FC) %>% distinct(gene, .keep_all = T)

cf_spermatogonia<-CF_forvenn%>%filter(stage_cf=="spermatogonia")
pf_spermatogonia<-PF_forvenn%>%filter(stage_pf=="spermatogonia")

cf_meiosis<-CF_forvenn%>%filter(stage_cf=="meiosis")
pf_meiosis<-PF_forvenn%>%filter(stage_pf=="meiosis")

cf_elongating<-CF_forvenn%>%filter(stage_cf=="elongating")
pf_elongating<-PF_forvenn%>%filter(stage_pf=="elongating")

###################unique markers per stage

spermatogonia_cfpfV <- list(
  CF=cf_spermatogonia$gene_name,
  PF=pf_spermatogonia$gene_name)

spermatogonia_cfpfVenn<-ggVennDiagram(spermatogonia_cfpfV)
spermatogonia_cfpfVenn

meiosis_cfpfV <- list(
  CF=cf_meiosis$gene_name,
  PF=pf_meiosis$gene_name)

meiosis_cfpfVenn<-ggVennDiagram(meiosis_cfpfV)
meiosis_cfpfVenn

elongating_cfpfV <- list(
  CF=cf_elongating$gene_name,
  PF=pf_elongating$gene_name)

elongating_cfpfVenn<-ggVennDiagram(elongating_cfpfV)
elongating_cfpfVenn

spermatogonia_cfpfVenn|meiosis_cfpfVenn|elongating_cfpfVenn

```

Now the same but only with top 100 markers

```{r}

cf.markerstop100_signif<-cf.top100 %>%
  filter(p_val_adj<0.05)
cf.markerstop100_signif<-merge(cf.markerstop100_signif,NewAnn2, by.x="gene",by.y="V2", all.x=T, all.y=F)
cf.markerstop100_signif<-na.omit(cf.markerstop100_signif)

pf.markerstop100_signif<-pf.top100 %>%
  filter(p_val_adj<0.05)
pf.markerstop100_signif<-merge(pf.markerstop100_signif,NewAnn2, by.x="gene",by.y="V2", all.x=T, all.y=F)
pf.markerstop100_signif<-na.omit(pf.markerstop100_signif)
...
CF_forvenn<-merge(cf.markerstop100_signif,stages_cf, by.x = "cluster", by.y = "cluster_cf", all = T)
CF_forvenn$stage_cf<-as.factor(CF_forvenn$stage_cf)
#PF
...
PF_forvenn<-merge(pf.markerstop100_signif,stages_pf, by.x = "cluster", by.y = "cluster_pf", all = T)
PF_forvenn$stage_pf<-as.factor(PF_forvenn$stage_pf)
